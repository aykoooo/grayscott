...
pub fn generateWgslSubgroupShuffle(buf: []u8, w: u32, h: u32, tile_x: u32, tile_y: u32) ![]const u8 {
    const stride: u32 = tile_x + 2;
    const rows: u32 = tile_y + 2;
    const tile_n: u32 = stride * rows;
    return std.fmt.bufPrint(buf,
        ...
        \\    let interior_row = lid.y > 0u && lid.y < TY - 1u;
        \\    if (interior_col && interior_row) {{
        \\        let u_l = subgroupShuffleDown(u_c, 1u);
        \\        let u_r = subgroupShuffleUp(u_c, 1u);
        \\        let u_t = tile_u[(lid.y) * STRIDE + (lid.x + 1u)];
        \\        let u_b = tile_u[(lid.y + 2u) * STRIDE + (lid.x + 1u)];
        \\        let u_ne = tile_u[(lid.y) * STRIDE + (lid.x + 2u)];
        \\        let u_nw = tile_u[(lid.y) * STRIDE + (lid.x)];
        \\        let u_se = tile_u[(lid.y + 2u) * STRIDE + (lid.x + 2u)];
        \\        let u_sw = tile_u[(lid.y + 2u) * STRIDE + (lid.x)];
        \\        let card_u = u_l + u_r + u_t + u_b;
        \\        let lap_u = fma(u_ne + u_nw + u_se + u_sw, 0.05, fma(card_u, 0.2, -u_c));
        \\        let v_l = subgroupShuffleDown(v_c, 1u);
        \\        let v_r = subgroupShuffleUp(v_c, 1u);
        \\        let v_t = tile_v[(lid.y) * STRIDE + (lid.x + 1u)];
        \\        let v_b = tile_v[(lid.y + 2u) * STRIDE + (lid.x + 1u)];
        \\        let v_ne = tile_v[(lid.y) * STRIDE + (lid.x + 2u)];
        \\        let v_nw = tile_v[(lid.y) * STRIDE + (lid.x)];
        \\        let v_se = tile_v[(lid.y + 2u) * STRIDE + (lid.x + 2u)];
        \\        let v_sw = tile_v[(lid.y + 2u) * STRIDE + (lid.x)];
        \\        let card_v = v_l + v_r + v_t + v_b;
        \\        let lap_v = fma(v_ne + v_nw + v_se + v_sw, 0.05, fma(card_v, 0.2, -v_c));
        \\        let uvv = u_c * v_c * v_c;
        \\        let u_next = u_c + params.dt * (params.da * lap_u - uvv + params.feed * (1.0 - u_c));
        \\        let v_next = v_c + params.dt * (params.db * lap_v + uvv - (params.feed + params.kill) * v_c);
        \\        let out_idx = y * WIDTH + x;
        ...
        \\    let u_se     = tile_u[(lid.y + 2u) * STRIDE + (lid.x + 2u)];
        \\    let u_sw     = tile_u[(lid.y + 2u) * STRIDE + (lid.x)];
        \\    let card_u = u_left + u_right + u_up + u_down;
        \\    let lap_u = fma(u_ne + u_nw + u_se + u_sw, 0.05, fma(card_u, 0.2, -u_c));
        \\    let v_left   = tile_v[(lid.y + 1u) * STRIDE + (lid.x)];
        \\    let v_right  = tile_v[(lid.y + 1u) * STRIDE + (lid.x + 2u)];
        \\    let v_up     = tile_v[(lid.y) * STRIDE + (lid.x + 1u)];
        ...
        \\    let v_se     = tile_v[(lid.y + 2u) * STRIDE + (lid.x + 2u)];
        \\    let v_sw     = tile_v[(lid.y + 2u) * STRIDE + (lid.x)];
        \\    let card_v = v_left + v_right + v_up + v_down;
        \\    let lap_v = fma(v_ne + v_nw + v_se + v_sw, 0.05, fma(card_v, 0.2, -v_c));
        \\    let uvv = u_c * v_c * v_c;
        \\    let u_next = u_c + params.dt * (params.da * lap_u - uvv + params.feed * (1.0 - u_c));
        \\    let v_next = v_c + params.dt * (params.db * lap_v + uvv - (params.feed + params.kill) * v_c);
...
pub fn generateWgslF16(buf: []u8, w: u32, h: u32, tile_x: u32, tile_y: u32) ![]const u8 {
    const stride: u32 = tile_x + 2;
    const rows: u32 = tile_y + 2;
    const tile_n: u32 = stride * rows;
    return std.fmt.bufPrint(buf,
